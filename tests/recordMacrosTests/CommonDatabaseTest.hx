...
import sys.db.*;
import sys.db.Types;
import recordMacrosTests.MySpodClass.ClassWithStringId;
import recordMacrosTests.MySpodClass.ClassWithStringIdRef;
import recordMacrosTests.MySpodClass.ComplexClass;
import recordMacrosTests.MySpodClass.OtherSpodClass;
import recordMacrosTests.MySpodClass.NullableSpodClass;
import recordMacrosTests.MySpodClass.IssueC3828;
import recordMacrosTests.MySpodClass.Issue6041Table;
import recordMacrosTests.MySpodClass.Issue19SpodClass;
import recordMacrosTests.MySpodClass.PublicKeyClass;
import recordMacrosTests.MySpodClass.PublicKeyRelationClass;
import recordMacrosTests.MySpodClass.SpodEnum;
...
class CommonDatabaseTest extends utest.Test {
	...
	static var testClasses:Array<{manager:Manager<Dynamic>}> = [
		MySpodClass, OtherSpodClass, NullableSpodClass, ClassWithStringId, ClassWithStringIdRef, IssueC3828, Issue6041Table, Issue19SpodClass, PublicKeyClass,
	...
	function assertIsInstanceOf(v:Dynamic, t:Dynamic, ?pos:haxe.PosInfos) {
		Assert.isTrue(Std.isOfType(v, t), pos);
	}
	...
	function getDefaultClass() {
		...
		scls.bytes = Bytes.ofString("\x01a");
	...
	function getDefaultNull() {
		...
		scls.string = "some string";
		#if hxnodejs
		scls.date = Date.fromTime(DateTools.makeUtc(2012, 7, 30, 0, 0, 0));
		#else
		scls.date = new Date(2012, 7, 30, 0, 0, 0);
		#end
		scls.abstractType = "other string";
	...
	public function testEnum() {
		...
		Manager.cleanup();
		var r1s = [
			for (c in MySpodClass.manager.search($anEnum == SecondValue, {orderBy: theId}))
				c.theId
		];
		...
		var ids = [id1, id2, id3];
		var s = [
			for (c in MySpodClass.manager.search($anEnum == SecondValue || ($theId in ids)))
				c.theId
		];
	...
	public function testSpodTypes() {
		...
		#if (haxe_ver > 3.407)
		var time = new Date(2012, 7, 30, 0, 0, 0).getTime();
		var savedTime = cls1.date.getTime();
		Assert.equals(time, savedTime);
		#else
		...
		Assert.equals("\x01a", cls1.bytes.toString());
